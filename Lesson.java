...
public class Lesson {
    ...
    public static void playerTurn() {
        int x, y;
        do {
            System.out.println("Введи координаты через пробел. Например X Y");
    ...
    public static boolean isWin(char player) {
        ...
        //диагональ слева направо, если диагональ начинается не с первого элемента
        dots = 0;
        for(int x = 0; x < DOTS_TO_WIN; x++) {
            for(int i = 1; i <= SIZE - DOTS_TO_WIN; i++) {
                if(map[x][x + i] == player) dots++;
                if(dots == DOTS_TO_WIN) return true;
            }
        }
